  ...
  if (Serial.available()) {
    ...
    if (input.startsWith("1,") || input.startsWith("2,")) {
      ...
      if(activate){
        ...
        int move_l1 = (current_angle_link1 - final_theta1)*13.333;
        int move_l2 = (current_angle_link2 - final_theta2)*13.333;
        int move_l3 = 45 - final_theta3;
        Serial.printf("move1=%d°, move2=%d°, move3=%d°\n", move_l1, move_l2, move_l3);
        String command = "";
        
        command = "s1," + String(move_l3);
        handleServoCommand(command);
        current_angle_link3 = final_theta3;

        if(move_l2 > 0){
          command = "2,+" + String(move_l2);
        }else{
          command = "2," + String(move_l2);
        }
        handleStepperCommand(command);
        current_angle_link2 = final_theta2;

        if(move_l1 > 0){
          command = "1,+" + String(move_l1);
        }else{
          command = "1," + String(move_l1);
        }
        handleStepperCommand(command);
